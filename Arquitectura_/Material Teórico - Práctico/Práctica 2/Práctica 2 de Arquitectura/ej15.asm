; Escribir un programa que implemente un conteo regresivo a partir de un 
; valor ingresado desde el teclado. 
; El conteo debe comenzar al presionarse la tecla F10.
; El tiempo transcurrido debe mostrarse en pantalla, actualizándose
; el valor cada segundo.

; costantes 
EOI EQU 20h ; end out in 
IMR EQU 21h 
IMRMASK EQU 11111110b ; activo la f10 
F10 EQU 24h 
F10ID EQU 24 
TIMER EQU 25h 
TIMERID EQU 25
...
org 1000h
 flag db 0
 cantN dw 0 
 num db 0 ; numer que ingreso 

org 96 ; f10 interrupcion
   dw F10SR

org 100 ; timer interrupcion
dw TIMERSR
...
; da inicio al timer
F10SR: in al, IMR
xor al, 00000011b ; le aplico esta máscara 
out IMR, al ; saco 
; incia contador del timer
mov al, 0
out RCONT, al
; fin de la iterrupcion
mov al, EOI
out EOI, al
iret
